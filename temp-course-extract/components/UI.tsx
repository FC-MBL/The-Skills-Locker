...
export const FileUpload: React.FC<FileUploadProps> = ({ label, accept, onChange, currentPreviewUrl, className = '', readContent = true }) => {
    ...
    const processFile = (file: File) => {
        if (!readContent) {
            onChange(file, ''); // Skip reading content
            return;
        }
...
export const Toast: React.FC<ToastProps> = ({ message, type = 'success', onClose }) => {
    ...
    return (
        <div className={`fixed bottom-6 right-6 ${bgColors[type]} text-white px-6 py-4 rounded-xl shadow-elevated flex items-center gap-3 z-50 animate-in fade-in slide-in-from-bottom-4`}>
            {icons[type]}

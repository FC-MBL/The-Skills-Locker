...
import { Badge, Button, Card } from '../../components/builder/UI';
import { Check, X, Eye, FileText } from 'lucide-react';

export const Submissions: React.FC = () => {
  const { courses, publishCourse, rejectCourse } = useCourseBuilder();
          ...
          submissions.map(course => (
            <Card key={course.id} className="p-6 flex flex-col md:flex-row md:items-center gap-6">
              ...
              <div className="flex items-center gap-3">
                <Button variant="ghost" size="sm" icon={FileText} onClick={() => navigate(`/admin/courses/${course.id}`)}>Metadata</Button>
                <Button variant="ghost" size="sm" icon={Eye} onClick={() => navigate(`/admin/courses/${course.id}/builder`)}>Preview</Button>
